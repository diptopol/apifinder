...
public abstract class GeneratedMethodArgTypeInformationImpl implements MethodArgTypeInformation {
    ...
    @Override
    public Optional<MethodInformation> findMethodId(Manager<MethodInformation> foreignManager) {
        if (getMethodId().isPresent()) {
            return foreignManager.stream().filter(MethodInformation.ID.equal(getMethodId().getAsLong())).findAny();
        } else {
